...
export default function ChallengesScreen({ skipAnimations = false }: ChallengesScreenProps) {
  ...
  const [showHonestLine, setShowHonestLine] = useState(skipAnimations); // "Let's be honest about networking -"
  const [showHateLine, setShowHateLine] = useState(skipAnimations); // "most people hate it."
  const [showSuckLine, setShowSuckLine] = useState(skipAnimations); // "Or suck at it."
  const [showBothLine, setShowBothLine] = useState(skipAnimations); // "Or both."
  const [fadeOutHeader, setFadeOutHeader] = useState(skipAnimations); // Fade out the header after impact
  const [showSubtitle, setShowSubtitle] = useState(skipAnimations); // Question subtitle
  const [showRecorder, setShowRecorder] = useState(skipAnimations);
  const [showExamples, setShowExamples] = useState(skipAnimations); // Examples below recorder
  const [showSkipButton, setShowSkipButton] = useState(skipAnimations);
  const [sequenceStarted, setSequenceStarted] = useState(skipAnimations);
  ...
  }, [sequenceStarted, skipAnimations]); // timing is stable, doesn't need to be in dependencies
      ...
      <Box sx={{ 
        ...
        <Box sx={{ maxWidth: 800, mx: 'auto' }}>
          ...
          <Box sx={{ 
            ...
            opacity: (fadeOutHeader && !skipAnimations) ? 0 : 1,
