...
-- 1. Actions table: user_id + goal_id + status (for dashboard widgets and goal details)
...
-- 2. Goal contacts: user_id + goal_id + status (for active contact queries)
...
-- 3. Goal milestones: user_id + goal_id + status (for milestone completion tracking)
...
-- 4. Goals table: user_id + status + is_primary (for dashboard active goals)
...
-- 5. Goals table: user_id + created_at (for chronological ordering)
...
-- 6. Actions covering index: includes commonly selected columns for dashboard widgets
...
-- 7. Milestones covering index: includes commonly selected columns
...
-- 8. Active goals only (most common filter)
...
-- 9. Overdue actions (for dashboard alerts)
